...
async def main(broker_host, broker_port, token):
    ...
    sub_client = pyqtt.Client("clientgonnasub", session_expiry_interval=600, maximum_packet_size=65535)
    ...
    sub_client.subscribe([pyqtt.Subscription('TEST/+', qos=1), pyqtt.Subscription('TEST', qos=0)],
                         subscription_identifier=2)

    pub_client = pyqtt.Client("clientgonnapub")
    ...
    msg = pyqtt.Message('', '42 is the answer again', qos=1, content_type='utf-8',
                        message_expiry_interval=60, topic_alias=42, user_property=('time', str(time.time())))
