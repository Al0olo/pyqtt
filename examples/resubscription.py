...
def on_subscribe(client, mid, qos, properties):
    ...
    for subscription, granted_qos in zip(subscriptions, qos):
        ...
        if granted_qos >= pyqtt.constants.SubAckReasonCode.UNSPECIFIED_ERROR.value:
...
async def main(broker_host, broker_port, token):
    ...
    sub_client = pyqtt.Client("clientgonnasub")
    ...
    subscriptions = [
        pyqtt.Subscription('TEST/PROPS/#', qos=1),
        pyqtt.Subscription('TEST2/PROPS/#', qos=2),
    ]
