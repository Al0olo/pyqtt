...
async def init_clients():
    ...
    aclient = pyqtt.Client(PREFIX + "myclientid", clean_session=True)
    aclient.set_auth_credentials(username)
    callback = Callbacks()
    callback.register_for_client(aclient)

    bclient = pyqtt.Client(PREFIX + "myclientid2", clean_session=True)
...
async def test_basic_subscriptions(init_clients):
    ...
    subscriptions = [
        pyqtt.Subscription(TOPICS[1], qos=1),
        pyqtt.Subscription(TOPICS[2], qos=2),
    ]
    bclient.subscribe(subscriptions, user_property=('key', 'value'), subscription_identifier=1)

    bclient.subscribe(pyqtt.Subscription(TOPICS[3], qos=1), user_property=('key', 'value'), subscription_identifier=2)
    await asyncio.sleep(1)
...
async def test_will_message(init_clients):
    ...
    will_message = pyqtt.Message(TOPICS[2], "I'm dead finally")
    aclient = pyqtt.Client(PREFIX + "myclientid3", clean_session=True, will_message=will_message)
    aclient.set_auth_credentials(username)
...
async def test_shared_subscriptions(init_clients):
    ...
    pubclient = pyqtt.Client(PREFIX + "myclient3", clean_session=True)
...
async def test_assigned_clientid():
    noidclient = pyqtt.Client("", clean_session=True)
...
async def test_redelivery_on_reconnect(init_clients):
    ...
    disconnect_client = pyqtt.Client(PREFIX + 'myclientid3', optimistic_acknowledgement=False,
